...
logger = logging.getLogger("MegaPreScanTrainerV3")

BASE_DIR = Path(__file__).resolve().parent.parent
SPLITS_DIR = BASE_DIR / "data" / "processed" / "splits_v3"
OUTPUT_MODEL_DIR = BASE_DIR / "models" / "deberta_v3_prescan_v3"

class PromptSecurityDataset(Dataset):
...
def train_prescan_v3():
    print("=" * 80)
    print("  [GuardShield AI] Stage-1 Pre-Scan Fine-Tuning across Mega Benchmark (3,500 Samples)")
    print("=" * 80)
    ...
    logger.info(f"Loading Base Pre-trained DeBERTa-v3 Model: {model_name}")
    try:
    ...
    logger.info(f"Loading Mega Train Split ({train_path})")
    train_dataset = PromptSecurityDataset(train_path, tokenizer)
    logger.info(f"Loading Mega Validation Split ({val_path})")
    val_dataset = PromptSecurityDataset(val_path, tokenizer)
    ...
    logger.info(f"Starting High-Capacity Training Execution (3,500 samples, 3 epochs)...")
    ...
    meta = {
        "model_architecture": "DeBERTa-v3-small (86M)",
        "train_samples": len(train_dataset),
        "val_samples": len(val_dataset),
        "epochs": 3,
        "validation_metrics": eval_metrics
    }
    with open(OUTPUT_MODEL_DIR / "mega_training_summary.json", "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)
        
    print("=" * 80)
    print(f"  Stage-1 Pre-Scan V3 Model Fine-Tuning Successful!")
    print(f"  Model Saved: {OUTPUT_MODEL_DIR}")
    print("=" * 80 + "\n")
